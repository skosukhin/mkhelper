...
# AC_OPENMP.
# ---------------------------------------------------------------------
# Find a flag to enable OpenMP support.
...
# The flag is cached in the acx_cv_prog_[]_AC_LANG_ABBREV[]_openmp
# (e.g. acx_cv_prog_fc_openmp) variable.
# A successful result is either "none needed" or the actual
# compiler flag required to enable OpenMP support. A nonsuccessful
# result equals to "unsupported".
# The macro creates an output variable OPENMP_[]_AC_LANG_PREFIX[]FLAG
# (e.g. OPENMP_FCFLAG), which is empty if the result is either
# "none needed" or "unsupported". The variable is set before running
# ACTION-IF_SUCCESS.
#
# Known flags:
# Intel >= 16: -qopenmp
  ...
  [OPENMP_[]_AC_LANG_PREFIX[]FLAG=
   ...
   AS_IF([test "x$acx_cv_prog_[]_AC_LANG_ABBREV[]_openmp" != "xunsupported"],
     [AS_IF([test "x$acx_cv_prog_[]_AC_LANG_ABBREV[]_openmp" != "xnone needed"],
        [OPENMP_[]_AC_LANG_PREFIX[]FLAG=$acx_cv_prog_[]_AC_LANG_ABBREV[]_openmp])
      $1],
     [m4_default(
        [$2],
        [AC_MSG_FAILURE([unable to detect flag needed to enable OpenMP support for $[]_AC_CC[]])])])
   AC_SUBST([OPENMP_]_AC_LANG_PREFIX[FLAG])])

# ACX_OPENMP_MACRO_VAL([ACTION-IF-SUCCESS], [ACTION-IF-FAILURE = FAILURE])
# ---------------------------------------------------------------------
# Get the value of the preprocessor macro _OPENMP, which can be interpreted
# as a version of the OpenMP standard supported by the compiler.
# If successful, run ACTION-IF-SUCCESS (defaults to nothing), otherwise
# run ACTION-IF-FAILURE (defaults to failing with an error message).
# The result is cached in the acx_cv_prog_[]_AC_LANG_ABBREV[]_openmp_macro
# (e.g. acx_cv_prog_fc_openmp_macro) variable.
# A successful result is either "unknown" (if _OPENMP is defined by the
# preprocessor but the macro was unable to retrieve its value) or the
# actual value of the preprocessor macro _OPENMP. A nonsuccessful result
# equals to "unsupported". The result of ACX_OPENMP is supposed to be in
# []_AC_LANG_PREFIX[]FLAGS before calling this macro.
AC_DEFUN([ACX_OPENMP_MACRO_VAL],
  [AC_CACHE_CHECK([for the value of the preprocessor macro _OPENMP set by $[]_AC_CC[]],
   ...
   AS_IF([test "x$acx_cv_prog_[]_AC_LANG_ABBREV[]_openmp_macro" != unknown],
     ...
     [m4_default(
        [$2],
        [AC_MSG_FAILURE([the preprocessor used by $[]_AC_CC[] does not set the macro _OPENMP])])])])
...
# If disabled, run ACTION-IF-DISABLED (defaults to nothing), otherwise
# run ACTION-IF-ENABLED (defaults to failing with an error message).
# The result of the macro is either "yes" or "no".
# The result is cached in the acx_cv_prog_[]_AC_LANG_ABBREV[]_openmp_disabled (e.g.
...
AC_DEFUN([ACX_OPENMP_CHECK_DISABLED],
  [AC_CACHE_CHECK([whether OpenMP support is disabled for $[]_AC_CC[]],
   ...
   AS_IF([test "x$acx_cv_prog_[]_AC_LANG_ABBREV[]_openmp_disabled" = "xyes"],
     ...
     [m4_default(
        [$2],
        [AC_MSG_FAILURE([OpenMP support for $[]_AC_CC[] is enabled by default: set []_AC_LANG_PREFIX[]FLAGS accordingly to disable it])])])])
