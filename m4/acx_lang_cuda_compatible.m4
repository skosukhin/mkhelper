...
# -----------------------------------------------------------------------------
# Checks whether the compiler for the current language can link objects compiled
# with the CUDA compiler. Tries to compile a simple CUDA code with the CUDA
# compiler and link the resulting object into a program in the current language
...
void conftest_cuda_foo(){}]])
   ...
   AS_VAR_IF([acx_cache_var], [yes], [$1],
     [m4_default([$2],
        [AC_MSG_FAILURE([_AC_LANG compiler cannot link objects compiled with dnl
CUDA compiler])])])
...
# -----------------------------------------------------------------------------
# Checks whether the compiler for the current language can link objects compiled
# with the CUDA compiler from sources that require C++ standard library. Tries
# to compile a simple C++ code with the CUDA compiler and link the resulting
# object into into a program in the current language with the corresponding
# compiler.
#
# If successful, runs ACTION-IF-SUCCESS, otherwise runs ACTION-IF-FAILURE
# (defaults to failing with an error message).
...
AC_DEFUN([ACX_LANG_CUDA_COMPATIBLE_STDCXX],
  [m4_pushdef([acx_cache_var],
   ...
   AC_CACHE_CHECK(
     [whether _AC_LANG compiler can link objects compiled with CUDA compiler dnl
...
# compile FOO-C-CODE with the CUDA compiler and link the resulting object into a
# program in the current language with the corresponding compiler. The result is
# either "yes" or "no". If you need to run extra commands upon successful
