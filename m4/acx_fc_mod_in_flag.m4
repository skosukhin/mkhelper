# ACX_FC_MOD_IN_FLAG([ACTION-IF-SUCCESS], [ACTION-IF-FAILURE = FAILURE])
# ---------------------------------------------------------------------
...
# The flag is cached in the acx_cv_fc_mod_in_flag variable.
...
AC_DEFUN([ACX_FC_MOD_IN_FLAG],
  [AC_REQUIRE([AC_PROG_FC])
   AC_CACHE_CHECK([for Fortran flag to include modules from a specified directory], [acx_cv_fc_mod_in_flag],
     [AC_LANG_PUSH([Fortran])
      acx_cv_fc_mod_in_flag=unknown
      mkdir conftest.dir
      cd conftest.dir
      AC_COMPILE_IFELSE([[
      ...
      write (*,"(a)") "a"
        ...
        [cd ..
         acx_fc_mod_in_flag_FCFLAGS_save=$FCFLAGS
         for acx_fc_mod_in_flag in -M -I '-I ' '-M ' -p '-mod ' '-module ' '-Am -I'; do
           # Add the flag twice to prevent matching an output flag.
           FCFLAGS="$acx_fc_mod_in_flag_FCFLAGS_save ${acx_fc_mod_in_flag}conftest.dir ${acx_fc_mod_in_flag}conftest.dir"
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([], [[
      use conftest_module
      call conftest_routine]])],
             [acx_cv_fc_mod_in_flag=$acx_fc_mod_in_flag
              break])
         done
         FCFLAGS=$acx_fc_mod_in_flag_FCFLAGS_save])
      rm -rf conftest.dir
      AC_LANG_POP([Fortran])])
   AS_IF([test x"$acx_cv_fc_mod_in_flag" != xunknown],
     [$1],
     [m4_default(
        [$2],
        [AC_MSG_FAILURE([unable to detect flag needed to include modules from a specified directory])])])])
