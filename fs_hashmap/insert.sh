...
fn_notice ()
{
  test "x$silent" = xyes || echo "fshm (insert): $1" >&2
}

fn_error ()
{
  echo "fshm (insert): error: $2" >&2 && exit $1
}

fn_warn ()
{
  test "x$silent" = xyes || echo "fhsm (insert): warning: $1" >&2
}
...
for option; do
  ...
  case $option in
    --help | -h)
      cat <<_EOF
Usage: $0 [OPTION]... KEY VALUE

Inserts the KEY-VALUE pair into the hashmap. If the pair is successfully added
or if the hashmap already contains the KEY, prints path to the copy of the VALUE
inside the hashmap storage.

Positional arguments:
  KEY               path to a file representing the key
  VALUE             path to a directory representing the value
...
Exit codes:
    0  the KEY-VALUE pair is successfully inserted
  100  the hashmap already contains the KEY
...
${value+:} false || fn_error 2 "missing positional argument VALUE"
...
test -d "$value" || fn_error 1 "value directory '$value' not found"
...
while test $current_bucket_size -lt $fshm_bucket_size; do
  ...
  if mkdir "$key_dir" 2>/dev/null; then
    # Copy the value:
    src_dir=$value
    dst_dir="$key_dir/value"
    eval "$fshm_copy_cmd" || fn_error 1 "failed to copy the value into the hashmap"
    # Seal the value:
    chmod -R a-w "$dst_dir"
    # Create the log file if needed:
    fn_log_access "$key_dir/log" 'insert (new key)'
    # Copy the key:
    cp "$key_file" "$key_dir/key~"
    # Seal the key:
    chmod a-w "$key_dir/key~"
    mv "$key_dir/key~" "$key_dir/key"
    # Seal the directory:
    chmod a-w "$key_dir"
    fn_notice "key '$key' is successfully added to the hashmap with the value '$value'"
    echo "$dst_dir"
    exit 0
  else
    timeout_max_count=1
    ...
    while :; do
      if test -f "$key_dir/key"; then
        ...
        if cmp "$key_file" "$key_dir/key" >/dev/null 2>/dev/null; then
          # The keys are equal:
          fn_log_access "$key_dir/log" 'insert (existing key)'
          fn_notice "the hashmap already contains a value with the key '$key'"
          echo "$key_dir/value"
          exit 100
        else
        ...
        fn_error 2 "failed to add key '$key' to the hashmap: time limit exceeded"
        ...
        fn_error 2 "failed to add key '$key' to the hashmap for unknown reasons"
      fi
...
fn_error 2 "failed to add key '$key' to the hashmap: bucket size limit exceeded"
